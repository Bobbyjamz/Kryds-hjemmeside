...
const SS_KEY = "kryds-debug-password";
...
export default function DebugClient({
  adminUsername,
  debugPasswordConfigured,
}: {
  adminUsername: string;
  debugPasswordConfigured: boolean;
}) {
  ...
  // Hent gemt password fra sessionStorage ved load
  useEffect(() => {
  ...
  async function runTests(pw: string, alsoSendEmail: boolean) {
    setRunning(true);
    setError(null);
    try {
      const r = await fetch("/api/admin/debug", {
        ...
        body: JSON.stringify({ password: pw, sendTestEmail: alsoSendEmail }),
      });
    ...
    return (
      <main className="min-h-screen bg-black text-cream py-16 px-6">
        <div className="max-w-[440px] mx-auto">
          ...
          <h1 className="font-condensed font-black text-[40px] uppercase tracking-[-.01em] text-cream leading-none mb-2">
            Debug
          </h1>
          <p className="text-[13px] text-muted mb-8">
            Indtast debug-password for at køre systemtests.
          </p>

          {!debugPasswordConfigured && (
            <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-[2px]">
              <p className="text-[13px] text-red-300 leading-[1.6]">
                <span className="font-bold">DEBUG_PASSWORD er ikke sat i Vercel.</span><br />
                Tilføj den i Vercel → Settings → Environment Variables for at aktivere denne side.
              </p>
            </div>
          )}
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault();
              if (password.trim()) runTests(password.trim(), false);
            }}
            className="bg-gray border border-[rgba(242,238,230,0.07)] rounded-[2px] p-6"
          >
            <label className="block">
              ...
              <input
                ...
                placeholder="••••••••"
                autoFocus
                className="w-full bg-black border border-[rgba(242,238,230,0.12)] text-cream font-mono text-[15px] px-4 py-3 rounded-[2px] focus:border-yellow/60 focus:outline-none"
                disabled={running}
              />
            </label>
            ...
            <button
              ...
              className="mt-5 w-full bg-yellow text-black font-condensed font-extrabold text-[12px] tracking-[.12em] uppercase px-5 py-3 rounded-[2px] hover:bg-yellow/90 transition-colors disabled:opacity-50"
            >
              {running ? "Kører tests…" : "Lås op og kør tests"}
            </button>
          </form>

          <Link
            href="/admin"
            className="block mt-6 text-center text-[11px] font-condensed uppercase tracking-[.15em] text-muted hover:text-yellow transition-colors"
          >
            ← Tilbage til admin
          </Link>
        </div>
  ...
  return (
    <main className="min-h-screen bg-black text-cream py-12 px-6">
      <div className="max-w-[1100px] mx-auto">
        ...
        <div className="flex items-start justify-between mb-10 flex-wrap gap-4">
          <div>
            ...
            <p className="text-[13px] text-muted mt-3">
              Logget ind som <span className="text-cream">{adminUsername}</span>
              {report && (
                <> · Sidst kørt <span className="text-cream">{new Date(report.ranAt).toLocaleString("da-DK")}</span></>
              )}
            </p>
          </div>
          <div className="flex gap-2 flex-wrap">
            <Link
              href="/admin"
              className="border border-[rgba(242,238,230,0.15)] text-muted font-condensed font-bold text-[11px] tracking-[.12em] uppercase px-4 py-2 rounded-[2px] hover:text-cream transition-colors"
            >
              ← Admin
            </Link>
            <button
              onClick={logOut}
              className="border border-[rgba(242,238,230,0.15)] text-muted font-condensed font-bold text-[11px] tracking-[.12em] uppercase px-4 py-2 rounded-[2px] hover:text-red-400 transition-colors"
            >
              Lås
            </button>
          </div>
        ...
        <div className="bg-gray border border-[rgba(242,238,230,0.07)] rounded-[2px] p-5 mb-8 flex items-center justify-between flex-wrap gap-4">
          ...
          <button
            onClick={() => runTests(password, sendTestEmail)}
            disabled={running}
